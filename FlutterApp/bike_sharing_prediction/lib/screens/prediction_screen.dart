    ...
    if (_formKey.currentState!.validate()) {
      ...
      if (!isConnected) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('❌ Cannot connect to API. Make sure it\'s running at ${ApiConfig.getApiUrl()}'),
      ...
      final success = await provider.predictBikeRentals(
      ...
      if (!success) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('❌ Prediction failed: ${provider.error}'),
            backgroundColor: Colors.red,
          ),
        );
      }
